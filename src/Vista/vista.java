...
public class vista {
	public static void main(String[] args) {	
		...
		System.out.println("* 1.- Control de almacen: \t      *\n"
				+ "* 2.-Punto de venta\t              *");
		System.out.println("***************************************"); opG = leerIn.nextInt();
		switch(opG) {
		case 1:
		do {
			System.out.println("***************************************");
			System.out.println("* 1.- Agregar productos al inventario *\n"
					...
					+ "* 3.- Editar inventario \t      *\n");
			System.out.println("***************************************");
			System.out.println("Ingresa la opción: "); op = leerIn.nextInt();
			switch(op) {
			case 1: prod = new Producto();
				System.out.println("Ingresa el Nombre: "); nombreProducto = leerS.nextLine().trim();
				prod.setNombre(nombreProducto);
				System.out.println("Ingresa el ID: "); ID = leerS.nextLine().trim();
				prod.setID(ID);
				System.out.println("Ingresa el precio unitario: "); precio = leerDo.nextDouble();
				prod.setPrecio(precio);
				System.out.println("Ingresa la cantidad: "); cantidad = leerIn.nextInt();
			...
			case 2: 
				...
				break;
			case 3: 
				System.out.println("Para editar el inventario es necesario ingresar el ID del producto: "); ID = leerS.nextLine().trim();
				int pos = Inventario.buscarID(ID);
				if(pos>=0) {
					System.out.println("Se encontró el ID, por favor ingrese el numero de elementos nuevos: "); int nuevaCantidad = leerIn.nextInt();
					Inventario.inventario.get(pos).setNuevaCantidad(nuevaCantidad);
				}else {
			...
			do {
				...
				case 1: 
					if( Inventario.inventario.size()>0) {
						...
						if(posi >=0) {
							System.out.println("Producto: " + Inventario.inventario.get(posi).getNombre() + "\n Existencias: " + Inventario.inventario.get(posi).getCantidad());
							System.out.println("Ingresa la cantidad de productos a comprar: "); int cantProd = leerIn.nextInt();
